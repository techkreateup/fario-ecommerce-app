...
export const CartProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  // Ensure discount doesn't exceed total and defaults to 0 safely
  discountAmount = Math.min(Number(discountAmount) || 0, subTotal);
  ...
  const applyCoupon = async (code: string): Promise<boolean> => {
    try {
      ...
      setCoupon({
        ...
        value: Number(couponData.value || couponData.discount_value || 0),
