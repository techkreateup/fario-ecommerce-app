...
export const CartProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const placeOrder = async (newOrder: Order): Promise<boolean> => {
    try {
      ...
      // 🔐 SECURE: Get Session Token from localStorage (avoid getSession() hang)
      let authToken = '';
      try {
        const storageKey = Object.keys(localStorage).find(k => k.startsWith('sb-') && k.endsWith('-auth-token'));
        if (storageKey) {
      ...
      if (!authToken) {
        console.warn("⚠️ No access token found, falling back to Anon Key (Less Secure)");
      }
      ...
      toast.error(`System error: ${err.message || 'Connection failed'}`);
      return false;
    }
  };
    ...
    <CartContext.Provider value={{
      ...
      addReturn: async (req) => {
        try {
          ...
          toast.error(`Failed to submit return: ${e.message}`);
        }
      },
