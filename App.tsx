    ...
    <AnimatePresence>
      {isVisible && (
        <MotionButton
          ...
          onClick={scrollToTop}
          className="fixed bottom-24 md:bottom-32 right-20 md:right-24 z-40 bg-white text-fario-dark w-14 h-14 rounded-full flex items-center justify-center shadow-2xl border border-gray-100 hover:bg-fario-dark hover:text-white transition-colors group"
          aria-label="Scroll to top"
        >
          <div className="absolute inset-0 bg-fario-purple/10 rounded-full blur-xl opacity-0 group-hover:opacity-100 transition-opacity" />
          <ArrowUp size={24} className="relative z-10 group-hover:-translate-x-1 transition-transform" />
        </MotionButton>
      )}
    </AnimatePresence>
  );
