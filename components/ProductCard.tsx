...
const ProductCard = React.forwardRef<HTMLDivElement, ProductCardProps>(
    ({ product, viewMode = 'grid', onAddToCart, navigate }, ref) => {
                        ...
                        <div className="md:w-64 border-l border-gray-100 md:pl-6 flex flex-col justify-between">
                            <div className="space-y-1">
                                <div className="flex items-baseline gap-2">
                                    <span className="text-3xl font-black text-black tracking-tighter">₹{price.toLocaleString()}</span>
                                </div>
                                {originalPrice && (
                                    <div className="flex items-center gap-2">
                                        <span className="text-[11px] text-gray-400 line-through">M.R.P: ₹{originalPrice.toLocaleString()}</span>
        ...
        return (
            <MotionDiv
                ...
                exit={{ opacity: 0 }}
                onClick={() => navigate(`/products/${id}`)}
                className="group flex flex-col cursor-pointer bg-white rounded-3xl border border-gray-200 shadow-sm hover:border-fario-purple/30 hover:shadow-xl transition-all duration-500 overflow-hidden h-full"
                onMouseEnter={() => setIsHovered(true)}
                onMouseLeave={() => setIsHovered(false)}
                role="button"
                tabIndex={0}
            >
                ...
                <div className="relative aspect-square bg-gray-50 overflow-hidden border-b border-gray-100">
                    ...
                    {/* Status Badge */}
                    {originalPrice && (
                        <div className="absolute top-3 left-3 bg-rose-600 text-white px-2 py-1 rounded-sm text-[8px] font-black uppercase tracking-widest z-20 shadow-lg">
                            Limited time deal
                        </div>
                    )}
                ...
                {/* METADATA GRID UNIT (AMAZON STYLE HIGH DENSITY) */}
                <div className="p-4 flex flex-col flex-grow bg-white">
                    ...
                    <div className="mb-2">
                        <h3 className="text-sm font-bold text-black group-hover:text-fario-purple transition-colors line-clamp-2 leading-tight">
                            {name} ({gender}) | {tagline} | {description?.split('.')[0]}
                        </h3>
                    </div>

                    {/* Ratings & Sales Velocity */}
                    <div className="flex items-center gap-2 mb-3">
                        ...
                        <span className="text-[10px] font-medium text-gray-600">5K+ bought in past month</span>
                    </div>

                    {/* High Density Pricing */}
                    <div className="mt-auto space-y-1.5">
                        <div className="flex items-baseline flex-wrap gap-x-2">
                            <span className="text-2xl font-black text-black tracking-tight">₹{price.toLocaleString()}</span>
                            {originalPrice && (
                                <>
                                    <span className="text-xs text-gray-400 line-through">M.R.P: ₹{originalPrice.toLocaleString()}</span>
                        ...
                        <div className="bg-gray-50 p-2 rounded-lg border border-gray-100">
                            <p className="text-[9px] font-bold text-gray-600 leading-tight">
                                Up to 5% back with Fario Elite Prime Card
                            </p>
                        </div>

                        <div className="pt-2">
                            <p className="text-[11px] text-gray-800 leading-tight">
                                <span className="font-bold">FREE delivery</span> <span className="font-medium text-gray-600">Thu, 29 Jan</span>
                            </p>
                            <p className="text-[11px] text-gray-800">
                                Or fastest delivery <span className="font-bold">Tomorrow, 28 Jan</span>
                            </p>
                        </div>
                    </div>
                </div>
