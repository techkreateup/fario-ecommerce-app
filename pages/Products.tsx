...
import {
  Search, Package, ArrowUpRight,
  SlidersHorizontal, Check, X, RotateCcw, Palette, Ruler, Tag,
...
const Products: React.FC = () => {
  const traverseNavigate = useNavigate();
  ...
  return (
    <div className="pt-24 pb-20 min-h-screen bg-gray-50 font-sans text-gray-800 relative isolate">
      ...
      <div className="container mx-auto px-4 md:px-6 max-w-[1600px] relative">
          ...
          <div className="lg:col-span-9">
            {loading ? (
              <div className="flex flex-col items-center justify-center py-40 text-gray-300">
                <Loader2 className="animate-spin mb-4 text-fario-purple" size={32} />
                <p className="text-sm font-medium text-gray-500">Loading inventory...</p>
              </div>
              ...
              <div className={viewMode === 'grid' ? 'grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 gap-x-6 gap-y-10' : 'flex flex-col gap-6'}>
...
// Simplified Filter Section Accordion
const FilterSection: React.FC<{ title: string; subtitle?: string; children: React.ReactNode; isOpenDefault?: boolean }> = ({ title, subtitle, children, isOpenDefault = false }) => {
  const [isOpen, setIsOpen] = useState(isOpenDefault);

  return (
    <div className="bg-gray-50/50 p-6 rounded-[2.5rem] border border-gray-100/50 shadow-sm transition-all hover:bg-white hover:shadow-md group/section">
      <button
      ...
      >
        <div className="flex flex-col items-start gap-1">
          <div className="flex items-center gap-3">
            <div className={`w-1.5 h-4 rounded-full transition-all ${isOpen ? 'bg-fario-purple shadow-[0_0_8px_#7a51a0]' : 'bg-gray-200'}`} />
            <span className="font-black text-[11px] text-gray-900 uppercase tracking-[0.2em] italic group-hover:text-fario-purple transition-colors">{title}</span>
          </div>
          {subtitle && (
            <span className="text-[9px] text-gray-400 font-medium tracking-wide ml-6">{subtitle}</span>
          )}
        </div>
        <div className={`w-8 h-8 rounded-xl flex items-center justify-center transition-all ${isOpen ? 'bg-fario-purple/10 text-fario-purple rotate-180' : 'bg-white text-gray-300 shadow-sm'}`}>
          <ChevronDown size={14} />
        </div>
      </button>
