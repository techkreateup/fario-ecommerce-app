...
export default function Login() {
    ...
    const handleVerifyOTP = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // BTP: Local Bypass OR Specific Test User
            if (otp === '888888' && (
                window.location.hostname === 'localhost' ||
                window.location.hostname === '127.0.0.1' ||
                email === 'akashr.ad24@bitsathy.ac.in' || // Explicit User
                email === 'kreateuptech@gmail.com'       // Explicit Admin
            )) {
                console.log("🔓 Using Bypass OTP for:", email);
                // Try to get real user if exists, else return mock
                const { data: { user } } = await supabase.auth.getUser();
                verifyResult = {
                    data: { user: user || { id: 'test-user-id-' + Date.now(), email, role: 'authenticated' } },
                    error: null
                };
            } else {
