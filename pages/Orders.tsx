                            ...
                            <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-4 w-full md:w-auto">
                                <div className="relative w-full sm:w-64 md:w-auto">
                                    ...
                                    <input
                                        ...
                                        className="pl-10 pr-4 py-2 border border-gray-300 rounded-lg text-sm focus:border-fario-purple outline-none w-full"
                                    />
                                </div>
                                <button
                                    onClick={() => navigate('/products')}
                                    className="px-4 py-2 bg-gray-900 text-white rounded-lg text-sm font-bold uppercase tracking-wide w-full sm:w-auto text-center hover:bg-black transition-colors"
                                >
                                    Continue Shopping
                                </button>
                            </div>
                ...
                viewState === 'RETURNS' && selectedOrder && (
                    <div className="max-w-4xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-500">
                        ...
                        <div className="bg-white border border-gray-200 rounded-3xl overflow-hidden shadow-xl ring-1 ring-black/5">
                            <div className="p-4 md:p-8 border-b border-gray-100 bg-gradient-to-r from-gray-50 to-white flex flex-col md:flex-row gap-4 justify-between md:items-center items-start">
                                <div>
                                        ...
                                        <div className="flex items-center gap-4 mb-4">
                                            <div className="w-16 h-16 bg-gray-50 rounded-lg p-2 border border-gray-200 flex items-center justify-center">
                                                ...
                                                {selectedOrder.items?.[0]?.image ? (
                                                    <img src={selectedOrder.items[0].image} className="w-full h-full object-contain mix-blend-multiply" alt="Product" />
                                                ) : (
                                                    <Package className="text-gray-400" size={24} />
                                                )}
                                            </div>
                                            {/* @ts-ignore */}
                                            <p className="font-bold text-gray-900 text-sm max-w-[200px]">{selectedOrder.items?.[0]?.name || 'Unknown Product'}</p>
                                        </div>
