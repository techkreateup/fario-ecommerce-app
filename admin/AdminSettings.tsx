...
import {
    ...
    MoreVertical, LogOut, Activity, BarChart3,
    DollarSign, Mail, Lock, FileText, Zap
} from 'lucide-react';
...
const AdminSettings: React.FC = () => {
    ...
    const filteredTabs = () => {
        if (!searchQuery) return ['general', 'payment', 'team', 'shipping', 'notifications', 'security'];
        // Simple search logic
        const terms = searchQuery.toLowerCase();
        return [
            'general', 'payment', 'team', 'shipping', 'notifications', 'security'
        ].filter(tab => tab.includes(terms) || (tab === 'payment' && 'stripe'.includes(terms)));
    };
